...
def plot(df: pd.DataFrame, area: dict, save_to: str = None) -> None:
    ...
    fig, axs = plt.subplots(figsize=(7, 3))  # 700x700 px
    sns.set_theme()
    ...
    p1 = sns.histplot(data=df, x='day_of_week', hue='bids_ranges', color='skyblue', multiple='dodge')
    ...
    plt.gcf().text(.854, .200, _dt.utcnow().strftime('FRA,\n%F\n%T UTC'), fontsize=6, color='gray', ha='right')
